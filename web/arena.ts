...
import { KataGoArenaEngine, MokaArenaEngine } from "./engines";
...
const mokaBrierScoreElement = document.querySelector("#moka-brier-score");
const mokaWinsElement = document.querySelector("#moka-wins");
let runNumber = 0;
...
const didMokaWin = (experiment: GoArenaExperiment) =>
  experiment.result !== null &&
  ((experiment.result.winner === "black" && experiment.isMokaBlack) ||
    (experiment.result.winner === "white" && !experiment.isMokaBlack));

const getMokaWinProbability = (experiment: GoArenaExperiment, evaluation: GoArenaEvaluation) => {
  const isMokaTurn =
    (experiment.gameState.nextColor === GO_ARENA_BLACK_COLOR && experiment.isMokaBlack) ||
    (experiment.gameState.nextColor !== GO_ARENA_BLACK_COLOR && !experiment.isMokaBlack);
  return isMokaTurn ? evaluation.winProbability : 1 - evaluation.winProbability;
};
...
const createExperimentElements = (experiments: GoArenaExperiment[]) => {
  ...
  for (const experiment of experiments) {
    ...
    labelElement.textContent = `#${experiment.id + 1} · ${
      experiment.isMokaBlack ? "Moka B" : "Moka W"
    }`;
...
const renderExperiments = (experiments: GoArenaExperiment[]) => {
  for (const experiment of experiments) {
    ...
    if (labelElement && experiment.result) {
      labelElement.textContent = `#${experiment.id + 1} · ${
        didMokaWin(experiment) ? "Moka" : "KataGo"
      } +${Math.abs(experiment.result.score).toFixed(1)}`;
      labelElement.className = `mb-1 truncate text-[11px] ${
        didMokaWin(experiment) ? "text-neutral-900" : "text-neutral-400"
      }`;
    }
  }
};
...
const updateSummary = (experiments: GoArenaExperiment[]) => {
  ...
  const mokaWinCount = completedExperiments.filter((experiment) => {
    if (!experiment.result) {
      return false;
    }

    return didMokaWin(experiment);
  }).length;
  ...
  updateText(mokaWinsElement, String(mokaWinCount));
  updateText(kataGoWinsElement, String(completedExperiments.length - mokaWinCount));
...
const calculateBrierScore = (
  ...
  const squaredErrors = experiments.flatMap((experiment) => {
    ...
    const outcome = didMokaWin(experiment) ? 1 : 0;
...
const renderGameResults = (experiments: GoArenaExperiment[]) => {
  ...
  for (const experiment of experiments) {
    ...
    const winnerName = didMokaWin(experiment) ? GO_ARENA_MOKA_NAME : "KataGo";
    rowElement.append(
      createTableCell(String(experiment.id + 1), "py-2 pr-4 text-neutral-500"),
      createTableCell(experiment.isMokaBlack ? "Black" : "White"),
      createTableCell(formatProbability(experiment.mokaInitialWinProbability)),
...
const renderRunSummary = (summary: GoArenaRunSummary) => {
  updateText(mokaBrierScoreElement, summary.mokaBrierScore.toFixed(3));
  ...
  rowElement.append(
    ...
    createTableCell(String(summary.mokaWinCount)),
    createTableCell(String(summary.kataGoWinCount)),
    createTableCell(String(summary.moveCapCount)),
    createTableCell(summary.mokaBrierScore.toFixed(3)),
...
const completeExperiment = (experiment: GoArenaExperiment) => {
  ...
  const blackName = experiment.isMokaBlack ? GO_ARENA_MOKA_NAME : "KataGo";
  const whiteName = experiment.isMokaBlack ? "KataGo" : GO_ARENA_MOKA_NAME;
  experiment.result = {
    blackName,
...
const runExperiments = async (
  ...
  const experiments = Array.from(
    ...
    (_, experimentIndex): GoArenaExperiment => ({
      ...
      id: experimentIndex,
      isMokaBlack: experimentIndex % 2 === 0,
      kataGoInitialWinProbability: null,
      result: null,
      mokaInitialWinProbability: null,
    }),
  );
  ...
  updateText(mokaBrierScoreElement, "—");
  ...
  const [mokaEvaluations, kataGoEvaluations] = await Promise.all([
    mokaEngine.evaluateStates(gameStates),
  ...
  for (let experimentIndex = 0; experimentIndex < experiments.length; experimentIndex += 1) {
    ...
    experiment.mokaInitialWinProbability = getMokaWinProbability(
      experiment,
      mokaEvaluations[experimentIndex],
    );
    experiment.kataGoInitialWinProbability = getMokaWinProbability(
      experiment,
  ...
  while (experiments.some((experiment) => experiment.result === null)) {
    const mokaExperiments: GoArenaExperiment[] = [];
    ...
    for (const experiment of experiments) {
      ...
      const isMokaTurn =
        (experiment.gameState.nextColor === GO_ARENA_BLACK_COLOR && experiment.isMokaBlack) ||
        (experiment.gameState.nextColor !== GO_ARENA_BLACK_COLOR && !experiment.isMokaBlack);
      (isMokaTurn ? mokaExperiments : kataGoExperiments).push(experiment);
    }

    const [mokaMoves, kataGoMoves] = await Promise.all([
      mokaEngine.selectMoves(mokaExperiments.map((experiment) => experiment.gameState)),
      kataGoEngine.selectMoves(kataGoExperiments.map((experiment) => experiment.gameState)),
    ]);

    for (let moveIndex = 0; moveIndex < mokaExperiments.length; moveIndex += 1) {
      const experiment = mokaExperiments[moveIndex];
      const nextState = playMove(experiment.gameState, mokaMoves[moveIndex]);
  ...
  renderExperiments(experiments);
  updateSummary(experiments);
  const mokaWinCount = experiments.filter(didMokaWin).length;
  ...
  const summary: GoArenaRunSummary = {
    ...
    kataGoWinCount: experiments.length - mokaWinCount,
    moveCapCount,
    runNumber: (runNumber += 1),
    mokaBrierScore: calculateBrierScore(
      experiments,
      (experiment) => experiment.mokaInitialWinProbability,
    ),
    mokaWinCount,
  };
...
const initializeArena = async () => {
  ...
  const mokaEngine = new MokaArenaEngine();
  const kataGoEngine = new KataGoArenaEngine();

  try {
    await Promise.all([mokaEngine.initialize(), kataGoEngine.initialize()]);
    ...
    experimentForm.addEventListener("submit", (event) => {
      ...
      void runExperiments(experimentCount, mokaEngine, kataGoEngine)
        .catch((error) => {
          updateText(
            statusElement,
