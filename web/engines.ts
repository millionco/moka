...
import { encodeMokaFeatures, encodeTeacherFeatures, selectHighestLegalMove } from "./game";
import { GoModelWorkerClient } from "./client";

class MokaArenaEngine implements GoArenaEngine {
  name = GO_ARENA_MOKA_NAME;
  ...
  initialize = async () => {
    await this.client.initialize({
      manifestUrl: GO_ARENA_MOKA_MANIFEST_URL,
      weightsUrl: GO_ARENA_MOKA_WEIGHTS_URL,
    });
  };

  evaluateStates = async (gameStates: GoArenaGameState[]) => {
    const results = await Promise.all(
      gameStates.map((gameState) => this.client.infer(encodeMokaFeatures(gameState))),
    );
...
export { KataGoArenaEngine, MokaArenaEngine };
