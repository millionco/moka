...
const encodeMokaFeatures = (gameState: GoArenaGameState) => {
  const features = new Float32Array(GO_ARENA_BOARD_AREA * GO_ARENA_MOKA_INPUT_PLANE_COUNT);
  const visitedMoves = new Set<number>();
  const setFeature = (move: number, featureIndex: number, value = 1) => {
    features[move * GO_ARENA_MOKA_INPUT_PLANE_COUNT + featureIndex] = value;
  };
